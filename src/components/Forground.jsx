...
const Forground = () => {
  ...
  return (
    <div ref={ref} className="fixed z-[3] w-full h-full flex gap-5 flex-wrap p-5">
        <Button outline gradientDuoTone="tealToLime" className="w-30 h-10 text-white m-5 overflow-hidden font-bold flex items-center justify-center align-top fixed top-0 right-0">Add Notes</Button>
        {data.map((item, index) => (
            <Card key={index} item={item} referance={ref} addData={addData}/>
        ))}
    </div>
  )
}
